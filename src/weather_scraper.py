...
def get_weather_info():
    ...
    try:
        ...
        return {"temperature": temp, "humidity": humid, "dust": dust, "status": status}
...
def extract_temperature(driver):
    try:
        temp = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.CLASS_NAME, "tmp"))
        )
        return temp.text.strip()
    except:
        print("온도 다른 방법으로 다시 시도")

        element_selectors = [".temperature", ".temp", "[class*='tmp']", ".current-tmp"]
...
def extract_dust_info(driver):
    try:
        ...
        element_selectors = [".air-quality", ".dust-info", "[class*='air']", ".pm-info"]
        for selector in element_selectors:
            try:
                element = driver.find_element(By.CSS_SELECTOR, selector)
                dust = element.text.strip()
                if dust and any(
                    word in dust for word in ["좋음", "보통", "나쁨", "매우"]
                ):
                    return dust
            except:
                continue
